...
On first use, the UI extension can prompt to add `${windowFlashNotifyId}` and `${windowFlashNotifyAlert}` to the local `window.title` setting. Once enabled, each relay window shows a short ID such as `[WFN:3A7F]` in the title, and notifications use that ID as the highest-priority window match. `flash` notifications can also show a moving `!!` title alert in a fixed slot at the front of the title, such as `[!!--]`, `[-!!-]`, and `[--!!]`, so the title does not repeatedly grow and shrink while blinking. You can also run `Window Flash Notify: Enable Precise Window Matching` manually.
